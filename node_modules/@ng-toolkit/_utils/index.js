...
function addDependencyToPackageJson(tree, options, name, version, dev = false) {
...
function addImportStatement(tree, filePath, type, file) {
    ...
    let results = fileContent.match(new RegExp("import.*{.*(" + type + ").*}.*(" + file + ").*"));
    if (results) {
        return;
    }
...
function updateNgToolkitInfo(tree, options, newSettings) {
...
function editNgModule(tree, filePath, literal, add = true, entry) {
    ...
    if (literalNode) {
        let temp = literalNode; //compilation error work-around
    ...
    if (!literalNode && add && ngModuleDecorator) {
        let ngModuleContent = ngModuleDecorator.arguments;
